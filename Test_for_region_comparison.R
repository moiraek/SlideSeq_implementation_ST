...
# Moira Ek
# 2019 07 22
# A first idea/test regarding how to detect differential expression
#  between two selected regions - first calculate distance distributions
#  etc, only including the spots in the two regions. Then, calculate 
#  everything within each region --> If spatially non-random when both
#  are included, but not when only one a gene can be said to be
#  differentially expressed.
#------------------------------------------------------------------------



# Effektivare att inte lägga det som en funktion --> avståndsmatrisen 
#  behöver inte beräknas om? Då blir det dock ett större objekt
#  som måste kallas varje gång, och några ytterligare beräkningar...
setwd("/home/moiraek/summerp19/SlideSeq_etc/Till_git")

raw_data <- as.data.frame(t(read.table("Rep1_MOB_count_matrix-1.tsv", check.names=FALSE)))

# Remove genes with expression in less than 5 spots.
#data <- raw_data[rowSums(raw_data!=0)>4,]

reg_1 <- data[,which(data["2010300C02Rik",]!=0)]
reg_2 <- data[,which(data["2010300C02Rik",]==0)]

#Remove genes with expression in less than 3 spots in each of the test 
# regions
reg_1 <- 


# NB - look at what input parameters are desirable
Diff_expr <- function(testdata){
...
# Forts här - får felmeddelande för reg_1
...
both <- Diff_expr(data)
region_1 <- Diff_expr(reg_1)
region_2 <- Diff_expr(reg_2)
