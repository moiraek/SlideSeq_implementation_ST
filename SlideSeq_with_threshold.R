...
#     for this to work...
# 4. Attempt to introduce a threshold for the number of transcripts
#     that will earn the label "expressed" in the binary approach,
#     since weighting doesn't seem to be working very well...
#
#-----------------------------------------------------------------------
...
#--------------------------------------------------------------------
# Removing all counts under a specified threshold for the binary
#  classification (expressed/not expressed). Removing empty rows.
# Also removing genes with expression oabove the threshold in only
...
testdata <- testdata[rowSums(testdata!=0)>1,]
...
#  p-values here, this doesn't work (n would have to be increased).
